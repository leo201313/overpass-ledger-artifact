...
const memoryLDB = "LDB_MEMORY_MODE"

func NewSimpleMemLDB() *SimpleLDB {
	db, err := leveldb.Open(storage.NewMemStorage(), nil)
	if err != nil {
		panic(err)
	}

	slb := NewSimpleLDB(memoryLDB, db)
	return slb
}

func (sldb *SimpleLDB) BatchWrite(batch *leveldb.Batch) error {
	return sldb.db.Write(batch, nil)
}
